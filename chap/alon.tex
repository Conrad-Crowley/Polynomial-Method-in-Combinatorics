...
The Cauchy-Davenport Theorem (Theorem \ref{thm:C-D}) is indeed tight as illustrated by the following two examples.
...
\begin{example}
    ...
    \begin{align*}
        ...
        \intertext{ which produces ea}
        |A+B| &= m + n -1 = |A| + |B| -1
    \end{align*}
\end{example} \todo{fix example}
Choosing $A$ and $B$ to be singletons, $|A+B| =1$, achieving the bound.
