...
transforming tangencies between circles in the plane to intersections between their lifted curves in $\RR^3$. 
...
\begin{lemma}[Bezout Lemma in $\RR^3$]     \label{lem:Bezout}
...
\end{lemma}
To preserve the flow of this chapter, the proof of this result will be given in \hyperref[appendix:Bezout]{the appendix}.
...
\begin{proof}[Proof of Theorem \ref{thm:circle-tangencies}]
    ...
    allows us to reduce to a collection
     $\Gamma$ where each circle is tangent to  $\gtrsim N^{1/2}$ other circles. 
    ...
    We will now interpolate $\sim N^{3/2}$\todo{$\sim$ vs $\gtrsim$} incidence points with a minimal polynomial whose degree we can control, and show that if this contains too many intersections it must also contain the curves. 
    Then due to the tangent vectors spanning the $z$-axis at the incidence points we will be able to achieve a contradiction.

    For each $\beta(\gamma_i) \in \beta(\Gamma)$ define a collection $K_i$ as a set of $\sim N^{1/2}$ incidence points of $\beta(\gamma_i)$. Then $|\cup K_i| \lesssim N^{3/2}$.
    Let $P \in \RR[x,y,z]$ be a non-zero polynomial of minimal degree that vanishes on all the incidence points in $\cup K_i$. 
    This polynomial interpolates $\lesssim N^{3/2}$ points in $\RR^3$, so by Lemma \ref{lem:paramcounting}
    the degree of $P$ is $\lesssim \left(N^{3/2}\right)^{\frac{1}{3}} =  N^{1/2}$. \todo{check}
...
We can now define the following complementary sets based on whether the variety is contained entirely in $Z(P)$:\todo{colon here ok or no?}
\begin{align*}
...
lemma (Lemma \ref{lem:Bezout}).\todo{lemma or Lemma} Bounding the cardinality for $I(C_1,C_2)$ is again straightforward by Bézout. 
The interesting case here is $I(C_2,C_2)$, where we will be forced to argue via a recursive argument.

We begin with $I(C_1,C_1)$, that is, the intersections that occur between varieties not entirely contained in the zero set. We proceed by partitioning $I(C_1,C_1)$ according to whether the incidence occurs within a cell or on $Z(P)$, explicitly \todo{check english}
\begin{align*}
...
\section{Considerations for the case of Sphere Tangencies in $\RR^{3}$}[The Case of Sphere Tangencies in $\RR^{3}$]\todo{capitalisation?}
...
\begin{theorem}
    Let $\ES$ be a finite set of points in $\RR^3$ and $\Pi$ a finite set of non-collinear planes in $\RR^3$. Assume further that $|\ES|> |\Pi|$. \label{thm:point-plane-incidences}
     Then the incidence set $I(\ES, \Pi)$ obeys
    \[
        |I(\ES, \Pi)| \lesssim |\ES||\Pi|^{1/2}.
    \] 
\end{theorem} \todo{change this to be correct}
...
\begin{example}
 ...
 In \cite{ellenberg2016new}, the authors conjectured a bound for higher dimensional spheres: \todo{colon here?}
 \begin{conjecture}
