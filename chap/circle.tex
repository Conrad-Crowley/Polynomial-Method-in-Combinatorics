...
In general Lemma \ref{kaklem2}\todo{wrong lemma?} tells us that if we are to interpolate a set of $M$ points in $\RR^d$ by a polynomial $P$, the degree of $P$ is of order $\deg P \sim M^{1/d}$, 
...
To use the polynomial method here, we will need to ensure our tangencies
...
\begin{lemma}[Uniform Refinement]
    ...
    Then we can refine our set such that every circle in $\CE' \subset \CE$ is tangent to $\gtrsim N^{\alpha -1}$ circles. \label{lem:uniform-refine}
\end{lemma}
...
\begin{proof}
    ...
    This polynomial interpolates $\sim N^{3/2}$ points in $\RR^3$, so by Lemma \ref{kaklem2}\todo{wrong lemma?} 
