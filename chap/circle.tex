...
\begin{lemma}[Bézout's Lemma in $\RR^3$]     \label{lem:Bezout}
   Let $Z(P)$ be a surface over $\RR[X,Y,Z]$ and $Z(Q_1,Q_2)$ be a curve over $\RR[X,Y,Z]$. Suppose that $P,Q_1,Q_2$ have no pairwise common factors.
 Then we can bound the cardinality of the intersection of the zero sets by
    \[ |Z(P) \cap Z(Q_1,Q_2)| \leq \deg P \deg Q_1 \deg Q_2.\]
\end{lemma}
This result follows from Theorem 12.3 in Fulton's book ``\textit{Intersection Theory}'' (See \cite{Fulton1998}, page 223).
...
\begin{definition}
    An \textbf{algebraic variety} in $\RR^n$ is defined as $Z(Q_1,\dots, Q_k)$ for some non-zero polynomials $Q_1, \dots Q_k$. 
    The \textbf{dimension of a variety} $Z(Q_1,\dots, Q_k)$ is the dimension of the tangent spaces at its regular points (these are the points for which the matrix formed by the gradient vectors $\nabla Q_1, \dots, \nabla Q_k$ has maximal rank). 

    In particular, we refer to algebraic varieties of dimension 1 as \textbf{curves}.
\end{definition}
...
\begin{example}[Degenerate case in $\RR^{3}$, \cite{ZahlSpheres}]
    ...
    \begin{figure}[h]
        \centering 
        \includegraphics[width=\textwidth,trim={1cm 1cm 1cm 0.5cm},clip]{images/degenr3.png}
...
This is a somewhat weak conjecture, as the evidence to form it is simply the extension of the $\beta$-lifting technique into higher dimensions. The $\beta$-lifting of spheres in $\RR^d$ produces curves in $\RR^{2d-1}$, and it is the ratio of these dimensions that authors claim to be a bound for the exponent.
