...
one can rotate a needle by 180 degrees in the plane? Julius Pal\footnote{Westernized spelling, originally Gyula Pál.} answered this question in 1921 with the equilateral triangle. The 
problem without the convexity condition remained open. In 1924, after leaving the newly formed Soviet Union for Copenhagen, Besicovitch discovered this
...
If this limit does not exist, one can still define the upper and lower Minkowski dimension by taking the limit superior and limit inferior respectively.
We denote these as $\dim_{M_{\text{upper}}}$ and $\dim_{M_{\text{lower}}}$ respectively.
\end{definition}
...
\end{definition}
These dimensions are related by the inequality when they are all defined:
...
Bourgain produced one of the first non-trivial estimates of the dimension in 1991.\cite{BUSH1991} We present Guth's formulation here\cite{GUTH2016}:
...
By the pigeonhole principle, there is a point $x \in X$ which lies in at least $|\FF|M |X|^{-1}$ of the lines $l_i$.
...
\begin{proof}
    ...
    defined to be the set of lines $l_j$ which intersect $l_i$. An average point of $X$ lies in $|\FF|M |X|^{-1}$ lines $l_i$ . If each point of X was
    about average, then each hairbrush would contain $\gtrsim |\FF|^2 M |X|^{-1}$ lines. We claim
