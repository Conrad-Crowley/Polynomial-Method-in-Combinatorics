...
A Kakeya set in $\FP^n$ is a set that contains a line in every direction. The following section formalises the notion of a line in a finite set. The finite analogue to Conjectures \ref{conj:mink-kakeya} and \ref{conj:haus-kakeya} is:
...
so we have $|\FF|M$ points to distribute over $X$. 
...
\begin{theorem}[Hair Brush Argument]
    ...
    \]
\end{theorem}
\todo{do we need to define a vector space here?}
\begin{proof}
First we consider the case of $n = 2$. We prove a more general statement, which we will need for the $n>2$ case: Suppose that 
