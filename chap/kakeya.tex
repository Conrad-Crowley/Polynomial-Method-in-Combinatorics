...
\begin{conjecture}[Kakeya Conjecture in Finite Fields]
    If $A\subset \FP^n$ contains a line in every direction, then $|A| \geq \frac{1}{n!} |\FP|^n $. \label{KakeyaConjecture}
\end{conjecture}
...
In 2008, Dvir published a remarkably simple proof of Conjecture \ref{KakeyaConjecture} using basic facts about polynomials. 
...
Note that if $D=|\FF|-1$, and $|A| \leq {{|\FF|+n-1} \choose{|\FF|-1}} = { {|\FF|+n-1} \choose{n}}$ we have a polynomial of degree at most $|\FF|-1$ that vanishes on $A$. Since $\frac{|\FF|^n}{n!} < {{|\FF|+n-1} \choose{n}}$, we can definitely find such a polynomial when $|A| \leq \frac{|\FF|^n}{n!}$. The restriction of $D=|\FF|-1$ is somewhat necessary due to the fact that polynomials with a factor of $X^|\FF| - X$ vanish identically by Lemma \ref{lem:finite-fields-poly-identity}.
...
which consist in the terms of the expansion of the expression above obtained by the product of the $tz_i$ terms when $|\alpha| = D$. 
...
Now if $P_{|\ell}$ vanishes everywhere on $\ell$, since its dependence on $t$ is given by a polynomial of degree less than $|\FF|$, all its coefficients must be zero. In particular we must have $Q_{x,z,D} = 0$.
...
Notice that $Q_{x,z,D}$ no longer depends on $x$, but on $z$ alone. In particular $Q_D(z) = 0$, but $z$ was an arbitrary non-zero element of $\FF^n$, and $Q_D(z)$ also vanishes at zero, so it vanishes everywhere. Thus we can pick $\bar{P}$ = $Q_D$, and we are done.
}
...
We proceed by contradiction, assuming that $P$ vanishes everywhere. We let $x_1, \dots ,x_n$\todo{capitalisation of these?} be coordinates on $\FF^n$, and we write $P$ in the form
